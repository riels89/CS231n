...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        affine, cache1 = affine_forward(X, W1, b1)
        ReLU, cache2 = relu_forward(affine)
        affine, cache3 = affine_forward(ReLU, W2, b2)
        scores = affine
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        loss, dx4 = softmax_loss(affine, y)
    ...
    def loss(self, X, y=None):
        ...
        if self.use_batchnorm is True:
            ...
            for i in np.arange(0, self.num_layers-1):
                gamma.append(self.params[('gamma%d' % (i + 1))])
                beta.append(self.params[('beta%d' % (i + 1))])
            out = X
            cache = {}
            for i in np.arange(self.num_layers - 1):
                out, cache[i] = self.affine_batchnorm_relu_forward(out, w[i], b[i], gamma[i], beta[i], self.bn_params[i])

            scores, cache[len(cache)] = affine_forward(out, w[-1], b[-1])
            # print('test ' + str((cache[1])))
            ############################################################################
            #                             END OF YOUR CODE                             #
            ############################################################################
            ...
            for i in np.arange(self.num_layers):
                reg_loss += 0.5 * self.reg * np.sum(w[i] * w[i])
            loss = loss + reg_loss

            dx, dw, db = affine_backward(dx, cache[len(cache) - 1])
            dw += self.reg * w[self.num_layers - 1]
            grads['W' + str(self.num_layers)] = dw
            grads['b' + str(self.num_layers)] = db

            for i in np.arange(len(cache) - 1)[::-1]:
                dx, dw, db, dg, dbeta = self.affine_batchnorm_relu_backward(dx, cache[i])
                dw += self.reg * w[i]
                grads['W' + str(i + 1)] = dw
                grads['b' + str(i + 1)] = db
            ...
            cache = {}
            for i in np.arange(self.num_layers-1):
                out, cache[i] = affine_relu_forward(out, w[i], b[i], self.use_dropout, self.dropout_param)
            ...
            for i in np.arange(len(cache)-1)[::-1]:
                ...
                dx, dw, db = affine_relu_backward(dx, cache[i], self.use_dropout)
                dw += self.reg * w[i]
                grads['W' + str(i+1)] = dw
                grads['b' + str(i+1)] = db
            ############################################################################
